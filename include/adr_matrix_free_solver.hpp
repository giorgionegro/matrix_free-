...
{
  // Assemble RHS = f - A*lifting for non-homogeneous BC
  system_rhs = 0;
  lifting = 0;
  ...
  // Compute RHS = f - A*lifting using matrix-free evaluation
  ...
  for (unsigned int cell = 0; cell < matrix_free_storage->n_cell_batches(); ++cell)
  {
    ...
    for (unsigned int q = 0; q < fe_eval.n_q_points; ++q)
    {
      ...
      VectorizedArray<double> mu_value = {};
      VectorizedArray<double> gamma_value = {};
      VectorizedArray<double> f_value = {};
      Tensor<1, dim, VectorizedArray<double>> beta_value;
      for (unsigned int d = 0; d < dim; ++d)
        beta_value[d] = {};
    //This also need some work after removing OberverPOinters
      for (unsigned int v = 0; v < matrix_free_storage->n_active_entries_per_cell_batch(cell); ++v)
      {
        Point<dim> q_point;
        for (unsigned int d = 0; d < dim; ++d)
          q_point[d] = fe_eval.quadrature_point(q)[d][v];

        mu_value[v] = diffusion_coefficient.value(q_point);
        gamma_value[v] = reaction_coefficient.value(q_point);
        f_value[v] = right_hand_side.value(q_point);

        const Tensor<1, dim> beta_tensor = advection_field.value(q_point);
        for (unsigned int d = 0; d < dim; ++d)
          beta_value[d][v] = beta_tensor[d];
      }

      const auto transport_value = beta_value * gradient;
      const auto reaction_value = gamma_value * value;

      // Submit -A*lifting + f
      fe_eval.submit_gradient(-mu_value * gradient, q);
      fe_eval.submit_value(-transport_value - reaction_value + f_value, q);
    }
...
void ADRMatrixFreeSolver<dim, fe_degree>::solve()
{
  ...
  gmres_data.max_n_tmp_vectors = 200;
